...
const InteractiveDragon = () => {
  ...
  // SVG definitions for dragon parts
  const dragonParts = {
    head: (
      <g id="Cabeza">
        <path 
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = setTimeout(() => {
      ...
      // Reset pointer to center when resizing
      setPointer({ x: newDimensions.width / 2, y: newDimensions.height / 2 });
      // Reset radius to restart movement
      setRadius(0);
      // Detect mobile
  ...
  const handleTouchMove = useCallback((e: TouchEvent) => {
    ...
    if (touch) {
      const newPointer = { x: touch.clientX, y: touch.clientY };
      setPointer(newPointer);
      setRadius(0);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const N = 20; // Reduced from 40 to 20 segments for smaller dragon
    const initialSegments: DragonSegment[] = [];
    ...
    setPointer({ x: dimensions.width / 2, y: dimensions.height / 2 });
  }, [dimensions.width, dimensions.height]);

  // Animation loop - now properly handles resize and pointer tracking
  useEffect(() => {
    if (!screenRef.current || segments.length === 0 || dimensions.width === 0) return;

    const animate = () => {
      // Check if we still have valid dimensions and elements
      if (!screenRef.current || dimensions.width === 0 || dimensions.height === 0) {
        animationRef.current = requestAnimationFrame(animate);
        return;
      }
      
      setFrame(prev => prev + 0.003);
      setRadius(prev => Math.min(prev + 1, Math.min(pointer.x, pointer.y) - 20));
      ...
      setSegments(prevSegments => {
        ...
        // Update head (first segment) - use current pointer position
        const head = newSegments[0];
        const ax = (Math.cos(3 * frame) * radius * dimensions.width) / dimensions.height;
        const ay = (Math.sin(4 * frame) * radius * dimensions.height) / dimensions.width;
        head.x += (ax + pointer.x - head.x) / 8; // Faster response
        head.y += (ay + pointer.y - head.y) / 8; // Faster response
        ...
        for (let i = 1; i < newSegments.length; i++) {
          ...
          segment.x += (prevSegment.x - segment.x + (Math.cos(angle) * (50 - i)) / 8) / 3; // Reduced spacing and faster response
          segment.y += (prevSegment.y - segment.y + (Math.sin(angle) * (50 - i)) / 8) / 3; // Reduced spacing and faster response
          
          const scale = (162 + 4 * (1 - i)) / 150; // Reduced scale by dividing by 150 instead of 50
          
          if (segment.use) {
            segment.use.setAttribute(
              'transform',
              `translate(${(prevSegment.x + segment.x) / 2},${(prevSegment.y + segment.y) / 2}) rotate(${
                (180 / Math.PI) * angle
      ...
      // Auto-return to center when radius is large (or always on mobile)
      if (isMobile || radius > 40) { // Always return to center on mobile
        setPointer(prev => ({
          x: prev.x + (dimensions.width / 2 - prev.x) * (isMobile ? 0.02 : 0.08), // Slower on mobile
          y: prev.y + (dimensions.height / 2 - prev.y) * (isMobile ? 0.02 : 0.08) // Slower on mobile
        }));
      }
  ...
  }, [segments.length, dimensions.width, dimensions.height, isMobile, pointer.x, pointer.y, frame, radius]); // Re-added pointer and animation state
  ...
  }, [handleResize, handlePointerMove, handleTouchMove]);

  // Create segment elements
  useEffect(() => {
    if (!screenRef.current || segments.length === 0) return;
    ...
    segments.forEach((segment, i) => {
      ...
      screenRef.current?.appendChild(use);
      
      // Update the segment with the use element
      setSegments(prev => {
        const newSegments = [...prev];
        newSegments[i].use = use;
        return newSegments;
      });
    });
  }, [segments.length]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-10">
      <svg
        ...
        style={{
          filter: 'sepia(20%) drop-shadow(0 0 10px rgba(209, 169, 84, 0.5))'
        }}
      >
        <defs>
          {dragonParts.head}
